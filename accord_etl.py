...
def parse_args():
    ...
    parser.add_argument("-d", "--dictionary",
                        help="file containing data dictionary")
        ...
        for row in self._data_dictionary:
            ...
            if len(values) == 1:
                ...
                self._map_phenotype_to_concept.append(
                    (conceptpath, i2b2code, i2b2vartype, dbgap_code_id, varname))
            else:
                for i, value in enumerate(values):
                    ...
                    if (i2b2conceptlabel == dbgap_code_id):
                        varcode = ''.join(filter(str.isalnum, i2b2concept))
                    else:
                        varcode = ''.join(
                            filter(str.isalnum, i2b2concept)) + dbgap_code_id
                    ...
                    self._map_phenotype_to_concept.append(
                        (conceptpath, i2b2code, "assertion", dbgap_code_id, varname))
            ...
            if i > 0:
                ...
                if visitdateformat == 1:  # Use date from column visitdatevarname
                    ...
                    if additionaldatediffvarname != "NA":
                        if self._data[i][self._variables[additionaldatediffvarname]].isnumeric():
                            timediff2 = float(
                                self._data[i][self._variables[additionaldatediffvarname]])
    ...
    # Collect facts for facts.csv
        ...
        for i in range(len(self._data)):
            if i > 0:
                ...
                rightstartdate = beginDate + \
                    datetime.timedelta(days=int(righttimediff * 365))
                leftstartdate = beginDate + \
                    datetime.timedelta(days=int(lefttimediff * 365))
